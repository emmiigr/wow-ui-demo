
TourGuide:RegisterGuide("Kalimdor (51)", "Un'Goro Crater (51-52)", "Alliance", function()
return [[
F Darnassus |N|You probably want to hearth to IF| |QID|10520|
T Prayer to Elune (Part 2) |N|Upstairs in the Temple of the Moon| |QID|3378|
T Rise of the Silithid |NODEBUG| |QID|162|
A March of the Silithid |QID|4493|
T Assisting Arch Druid Staghelm |N|In a tower in the Cenarion Enclave (34.8, 8.7)| |Z|Darnassus| |QID|10520|
A Un'Goro Soil |QID|3764|
h Darnassus |N|The inn is in the Craftsman's Terrace (67.2, 15.7)| |Z|Darnassus| |QID|3781|

F Ratchet |QID|4502|
A Volcanic Activity |N|Just south of the dock.| |QID|4502|
C The Stone Circle |N|The chest outside the hut.| |QID|3444|

b Booty Bay |QID|2874|
T Whiskey Slim's Lost Grog |QID|580|
T Deliver to MacKinley |N|House near the upside-down half-boat| |QID|2874|
T Rescue OOX-09/HL! |O| |N|On the second level of the docks. Just up the ramp next to the hanging shark, in the first house after the small bridge (28.3,76.3)| |QID|836|
T Rescue OOX-17/TN! |O| |N|On the second level of the docks. Just up the ramp next to the hanging shark, in the first house after the small bridge (28.3,76.3)| |QID|648|
T Rescue OOX-22/FE! |O| |N|On the second level of the docks. Just up the ramp next to the hanging shark, in the first house after the small bridge (28.3,76.3)| |QID|2767|

F Gadgetzan |QID|3444|
T Sprinkle's Secret Ingredient |QID|2641| |NORAF|
A Delivery for Marin |QID|2661|
T Delivery for Marin |QID|2661|
A Noggenfogger Elixir |QID|2662|
T Noggenfogger Elixir |QID|2662|
A Super Sticky |QID|4504|
T March of the Silithid |QID|4493|
A Bungle in the Jungle |QID|4496|

T The Stone Circle |N|South of Gadget at the ruins (52.7, 45.9).  Skip the follow-up.| |Z|Tanaris| |QID|3444|
]]

end)

